...
export const generateMemoLists = async () => {
  const postsDir = path.join(process.cwd(), "public", "markdown-memo", "posts");
  ...
  // posts 폴더의 파일들
  const postsFileNames = fs.existsSync(postsDir)
    ? fs.readdirSync(postsDir)
    : [];
  const postsFiles = postsFileNames.filter(
    (file) => file.endsWith(".mdx") || file.endsWith(".md")
  );

  // legacy 폴더의 파일들
  const legacyFileNames = fs.existsSync(legacyDir)
    ? fs.readdirSync(legacyDir)
    : [];
  const legacyFiles = legacyFileNames.filter(
    (file) => file.endsWith(".mdx") || file.endsWith(".md")
  );
  ...
  postsFiles.forEach((file) => {
    console.log("[Generate Memo Lists] Processing file (posts):", file);
    const fileName = file.replace(/\.(mdx|md)$/, "");
    const fileContent = fs.readFileSync(path.join(postsDir, file), "utf8");
    memoLists.push({
      ...extractMetadataFromMemo(fileName, fileContent, "posts"),
    });
  });

  // legacy 폴더 처리
  legacyFiles.forEach((file) => {
    console.log("[Generate Memo Lists] Processing file (legacy):", file);
    const fileName = file.replace(/\.(mdx|md)$/, "");
    const fileContent = fs.readFileSync(path.join(legacyDir, file), "utf8");
    memoLists.push({
      ...extractMetadataFromMemo(fileName, fileContent, "legacy"),
    });
  });
  console.log("[Generate Memo Lists] Writing to memo-lists.json");
  fs.writeFileSync(
    path.join(process.cwd(), "public", "memo-lists.json"),
    JSON.stringify(memoLists, null, 2)
  );
  ...
  fs.writeFileSync(
    path.join(process.cwd(), "public", "memo-tree.json"),
    JSON.stringify(memoTree, null, 2)
  );
  console.log("[Generate Memo Lists] Done");
};

const extractMetadataFromMemo = (fileName, fileContent, folder = "posts") => {
