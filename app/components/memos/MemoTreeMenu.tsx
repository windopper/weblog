...
function TreeMenuItem({ menu, level }: TreeMenuItemProps) {
  const [isOpen, setIsOpen] = useState(level < 2); // 기본적으로 2단계까지 열림
  ...
  const menuContent = (
    <div
      className={`
        ...
        ${hasChildren || hasFileName ? "cursor-pointer hover:bg-zinc-800/50" : "cursor-default"}
        ${level === 0 ? "font-medium" : ""}
    ...
    >
      ...
      <span
        className={`
          ...
          ${isActive ? "text-zinc-100 font-medium" : level === 0 ? "text-zinc-200" : "text-zinc-400"}
